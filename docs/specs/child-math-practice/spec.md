...
- **Constrained by:** ADR-0001, ADR-0002, RFC-0001
- **Brief:** none
- **Discovery:** none
- **Contract:** none
- **Shape:** mixed
...
and optional labeled diagram. It uses a quiet, focused interface inspired by
...
- Let the agent create or modify the curriculum catalog, invoke arbitrary tools,
  decide authorization, or write arbitrary database records.
...
- [ ] Model calls receive only the authenticated child's authorized learning
  context; child, catalog, model, and tool data remain data rather than
  instructions. Each answer event permits one generated-content request, with a
