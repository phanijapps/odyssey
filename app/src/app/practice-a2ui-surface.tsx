...
import {
  odysseyPracticeA2uiActionSchema,
  parseOdysseyPracticeA2uiDocument,
  type OdysseyPracticeA2uiAction,
  type OdysseyPracticeA2uiDocument,
...
const practiceCatalog = new Catalog(ODYSSEY_A2UI_CATALOG_ID, [
  OdysseyText,
  OdysseyColumn,
  OdysseyTextResponse,
]);
type PracticeSurfaceModel = SurfaceModel<typeof OdysseyText>;

/** Renders and dispatches only the fixed, validated Practice A2UI contract. */
export function OdysseyPracticeA2uiSurface({
  document,
  onSubmit,
}: {
  ...
  useEffect(() => {
    const processor = new MessageProcessor([practiceCatalog], (action) => {
      const parsed = odysseyPracticeA2uiActionSchema.safeParse({
        name: action.name,
    ...
    const subscription = processor.onSurfaceCreated((created) => {
      if (created.id === "odyssey-practice")
        setSurface(created as PracticeSurfaceModel);
    });
    try {
      processor.processMessages(
        parseOdysseyPracticeA2uiDocument(document).messages,
      );
    } catch {
      setError(true);
    }
